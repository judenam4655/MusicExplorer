...
//    }
    ...
    func saveAnnotation(lineIndex: Int, wordIndex: Int, text: String, marker: String?) {
        guard let id = currentTrack?.id else { return }
        annotationStore.save(trackId: id, lineIndex: lineIndex, wordIndex: wordIndex, noteText: text, marker: marker ?? "1")
    }
...
/// Wires everything together per the event flow we settled on:
///
///   PlaybackTrackerService
///      onTrackChanged -> upsert track, fetch/cache lyrics, load translation
///                         + song info, close out previous track's history
///      onPositionTick -> drives "current lyric line", updates history logger
///
/// UI (or a CLI/debug harness) should only need to talk to this class.
//final class AppServices {
//    let db: SQLiteDB
//    let tracker = PlaybackTrackerService()
//    let lyricsRepo: LyricsRepository
...
//
//        Task {
////            let lyrics = await lyricsRepo.lyrics(for: track)
//            let state = await lyricsRepo.lyrics(for: track)
//            await MainActor.run {
//                self.currentLyricsState = state
//                if case .success(let result) = state {
...
//    func saveSongInfo(_ text: String) {
//        guard let id = currentTrack?.id else { return }
//        songInfoStore.save(trackId: id, content: text)
//    }
//    
//    func saveCustomOriginal(lrcText: String) {
//        guard let id = currentTrack?.id else { return }
