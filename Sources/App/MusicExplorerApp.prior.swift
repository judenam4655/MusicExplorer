...
//    @AppStorage("lyricAlign") var align: TextAlignmentChoice = .center
//
//    // User-supplied image (shown top-right) and the color sampled from it,
//    // used to tint the view's background.
...
//    }
//
//    // Soft gradient sampled from the user's image, fading out toward the
//    // bottom so it reads as ambience rather than a flat color block and
//    // never fights with the lyric text contrast.
//    @ViewBuilder
//    private var backgroundWash: some View {
//        if let tint = backgroundTint {
//            LinearGradient(
//                colors: [tint.opacity(0.32), tint.opacity(0.10), Color.clear],
//                startPoint: .top,
//                endPoint: .bottom
//            )
...
//    }
//
//    // Top-right button: shows the user's image once picked, or an
//    // add-image placeholder before that. Tapping opens a file picker;
//    // right-click (or the X badge) removes the current image.
...
//            .background(Color.secondary.opacity(customImage == nil ? 0.08 : 0.0))
...
//        }
//        .buttonStyle(.plain)
//        .help(customImage == nil ? "Add an image" : "Change image")
//        .contextMenu {
//            Button("Choose Image...") { isImporterPresented = true }
//            if customImage != nil {
//                Button("Remove Image", role: .destructive) { clearImage() }
//            }
//        }
...
//        }
//
//        // Sample the dominant color off the main thread so a large image
//        // doesn't hitch the UI, then animate the background tint in once
//        // it's ready.
//        Task.detached(priority: .userInitiated) {
//            let color = Self.representativeColor(from: image)
//            await MainActor.run {
...
//    }
//
//    /// Samples the image down to its average color via Core Image's
//    /// CIAreaAverage filter -- a fast, reasonable stand-in for "the
//    /// representative color" without pulling in a full palette library.
